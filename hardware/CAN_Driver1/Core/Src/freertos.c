...
#define MOTOR_SPEED_PID_MAX_IOUT 15.0f
...
void MS1_Control(float speed,float angle)
{
 ...
 // TIM4->CCR1 = Map(-2000,2000,PWM_LOW_MIN,PWM_HIGH_MAX,PID_Calc(&motorpid_angle1,gyro1.ryp.z,angle));
	 ...
	 //angle_output = Map(-139,125,710,1280,angle);//舵机黄线
	
		if (angle>0)
	{
		angle_output = Map(0,140,1000,1300,angle);//舵机黄线
	}
	else
	{
 ...
 if((fabs(fb_angle1-angle ))<8 && fabs(angle)>5 )
	{
...
void MS2_Control(float speed,float angle)
{
	...
	*/ 
	...
	if(fabs(fb_angle2 - angle) <8 && fabs(angle)>5 )
	{

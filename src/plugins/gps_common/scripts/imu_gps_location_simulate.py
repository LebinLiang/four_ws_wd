...
def callback_gpsodom(data):
    ...
    raw_x  = data.pose.pose.position.x 
    raw_y = data.pose.pose.position.y
    bag_raw_x.append(copy.deepcopy(raw_x))
    bag_raw_y.append(copy.deepcopy(raw_y))
    if len(bag_raw_x)>10 :
        del bag_raw_x[0]
        del bag_raw_y[0]
    gpsodom_location[0] = -1*np.mean(bag_raw_x)
...
def  fuselocation_tf():
    ...
    while not rospy.is_shutdown(): 
        ...
        if (Dt!=0):
            NE_speed[0] = Dgps_distance[0]/Dt
            NE_speed[1] = Dgps_distance[1]/Dt
        ...
        if(math.fabs(Dgps_distance[0]) >0.005):
            output_location[0]+=Dgps_distance[0]
        if(math.fabs(Dgps_distance[1]) >0.005):
            output_location[1]+=Dgps_distance[1]

        fuse_path.header.frame_id = "world"
        fuse_path.header.stamp = rospy.Time.now()
        p = PoseStamped()
        p.pose.position.x =gpsodom_location[1]
        p.pose.position.y =gpsodom_location[0]
        p.pose.position.z = 0.0
        fuse_path.poses.append(p)
        Path_Pub.publish(fuse_path)

        br.sendTransform((gpsodom_location[1],gpsodom_location[0],0),tf.transformations.quaternion_from_euler(0,0,real_yaw),rospy.Time.now(),"base_link","world")

        odom.header.frame_id="world"
        odom.header.stamp = rospy.Time.now()
        odom.pose.pose.position.x = gpsodom_location[1]
        odom.pose.pose.position.y = gpsodom_location[0]
...
if __name__=="__main__":
    try:
        ...
        #rospy.Subscriber("/fix_vel",TwistStamped ,callback_vel)
